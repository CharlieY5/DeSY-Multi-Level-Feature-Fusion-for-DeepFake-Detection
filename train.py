...
DEFAULT_LOG_DIR = '/root/autodl-nas/DeSY/logs'
DEFAULT_DATA_DIR = './'  # 修改为当前目录
...
def train_low_level_vision(model, train_loader, val_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f'Epoch {epoch+1}/{args.epochs}')):
            # 检查batch内容
            if not isinstance(batch, dict) or 'frames' not in batch or 'label' not in batch:
                print(f"训练批次 {i} batch 错误: {batch}")
                continue
            try:
...
def train_mid_level_temporal(model, train_loader, val_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            print('保存最佳模型')
        # 每一轮都保存最新权重
        torch.save(model.state_dict(), os.path.join(args.save_dir, 'mid_level_temporal_latest.pth'))
        print('保存最新中层模型权重')
...
def main(args=None):
    ...
    # 强制使用融合训练集
    args.data_dir = 'train_fusion_data.json'
    ...
    low_level_path = os.path.join(args.save_dir, 'best_low_level_vision.pth')
    mid_level_path = os.path.join(args.save_dir, 'mid_level_temporal_best.pth')
    high_level_path = os.path.join(args.save_dir, 'high_level_semantic_best.pth')

    # 检查低级视觉模型权重文件是否存在
    if os.path.exists(low_level_path):
        print("检测到 best_low_level_vision.pth，跳过低级视觉模型训练")
    else:
        print("开始训练低级视觉模型...")
    ...
    if os.path.exists(mid_level_path):
        print("检测到 mid_level_temporal_best.pth，跳过中级时序模型训练")
    else:
        print("开始训练中级时序模型...")
    ...
    if os.path.exists(high_level_path):
        print("检测到 high_level_semantic_best.pth，跳过高级语义模型训练")
    else:
        print("开始训练高级语义模型...")

...
class MidLevelTemporalModel(nn.Module):
    def __init__(self, input_dim, weight_path=None):
        super(MidLevelTemporalModel, self).__init__()
        # 恢复为原始权重加载方式（如有权重路径参数应由外部传入）
        ...
        # 检查权重文件是否存在
        # if not os.path.exists(MID_LEVEL_PATH):
        #     raise FileNotFoundError(f'中层时序模型权重文件不存在: {MID_LEVEL_PATH}')
        # self.load_state_dict(torch.load(MID_LEVEL_PATH, map_location='cpu'))

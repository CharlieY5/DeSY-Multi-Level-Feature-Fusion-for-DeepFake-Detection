...
class UCF101Downloader:
    def __init__(self, data_dir, force_extract=False):
        ...
        self.videos_dir = '/root/autodl-tmp/DeSY/data/raw/videos/UCF-101/UCF-101'
...
def create_dataloader(data_dir, batch_size=8, num_workers=2, pin_memory=True, force_extract=False, num_frames=16, transform=None, **kwargs):
    # 新增：支持 json 数据源
    if isinstance(data_dir, str) and data_dir.endswith('.json') and os.path.exists(data_dir):
        dataset = FusionJsonDataset(data_dir, num_frames=num_frames, transform=transform)
        # 随机划分 8:2
        total = len(dataset)
    ...
    train_dataset = VideoDataset(
        ...
        video_dir='/root/autodl-tmp/DeSY/data/raw/videos/UCF-101/UCF-101',
